...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    ...
    generated = {
      prompt: `Practice (${topic.title}): A school has two clubs — 20 students in the chess club and 15 in the art club — and a friend insists that means 35 different students belong to a club, even though several students are in both. Using the idea behind "${topic.title}", walk through how you'd help them think about what's wrong with simply adding the two numbers, and explain your reasoning in 2-4 sentences about that situation.`,
      correctAnswer:
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise finite-math tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
