...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    generated = await chatJson<{
      ...
      `You generate a single introductory lambda-calculus practice problem for a curious beginner. The problem MUST be on the topic "${topic.title}" and at difficulty "${difficultyLabel}" (${difficulty.toFixed(
        1,
      )}/5). Test general understanding of the SUBJECT of the lambda calculus, not recall of any particular book, lecture, or course. The question MUST be fully self-contained and answerable by anyone who knows the discipline: do NOT reference "the lecture", "the text", "the course", "the example", "the case", or any named character or example a student would only recognize from a specific reading.\n\nSTRICT QUESTION RULES (no exceptions):\n- Every question must present a SPECIFIC, concrete lambda-calculus situation (e.g. applying the identity function λx. x to an argument a and asking what it reduces to and why, deciding whether x is bound or free in λy. x and what that means for renaming, applying the Church numeral "two" λf. λx. f (f x) to a function and explaining why it repeats it twice, using TRUE = λx. λy. x to pick between two options and explaining how the choice happens, or why (λx. x x)(λx. x x) keeps reducing to itself forever) and ask the student to APPLY the topic's idea to THAT situation — to judge, compare, explain, interpret, or decide. Explain ideas intuitively, in plain words — do NOT demand heavy symbolic manipulation or a memorized formula.\n- NEVER ask for a definition, a term, or what something is "called"; NEVER ask the student to recite an abstract formulation from any reading.\n- NEVER write a question whose answer is a single word, a single term, or a bare "yes"/"no". The answer must require reasoning that is HARD TO SHARE: the student must explain their thinking about the specific situation in 2-4 sentences.\n- The "correctAnswer" is the model answer (2-4 sentences) showing the operational reasoning a strong student would give for THIS situation. The "explanation" briefly says what earns full credit.\n\nRespond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}. Avoid these recent prompts: ${JSON.stringify(
    ...
    generated = {
      prompt: `Practice (${topic.title}): A friend applies the identity function λx. x to an argument a and insists the result must be more complicated than just a, since "something must happen." Using the idea behind "${topic.title}", walk through how you'd help them see what actually happens step by step, and explain your reasoning in 2-4 sentences about that situation.`,
      correctAnswer:
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise lambda-calculus tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
